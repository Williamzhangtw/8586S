...
#define Msg_delayms_ID_Num 10	//上面消息总和
...
//id											Is_enable		Is_ready		time				(*funproc)
  ...
  tm1650_1_msg, 					NO,		  		NO,					100,				Tm1650_1_show_ISR,	
	...
	hotter1321_poweron_msg,	NO,					NO,					200,				Hotter1321_power_on_scan_ISR,//
	hotter1321_realTemp_msg,NO,					NO,					100,				Hotter1321_realTemp_ISR,
  hotterctrl_poweron_msg, NO,					NO,					100,				Solder_1_poweron_ISR,
...
/*
//use for main
#define Msg_main_ID_Num 4	//上面消息总和
Msg_Delayms Msg_main[Msg_delayms_ID_Num] =      //消息，消息处理函数
{	
	0, NO, NO,	200		,	NULL ,
	1, NO, NO,	10		,	NULL ,
	2, NO, NO,	10		,	NULL ,
	3, NO, NO,	10		,	NULL,//NULL
};


void mainTask_ISR(void)//轮询方式进行触发
{
 	uint8_t id;	
  static uint16_t time[Msg_main_ID_Num]={0};
	for(id=0;id<Msg_main_ID_Num;id++) //遍睨所有消息结构体
	{
		
		
		
		if(time[id]-- == 0)//起到原函数中的 延时功能
		{
			Msg_main[id].Is_ready = YES;
			time[id] = Msg_main[id].time;
...
void Msg_main_process(void)
{
	...
	for(uint8_t  id=0;id<Msg_main_ID_Num;id++) 
	{
		if(Msg_main[id].Is_ready ==YES)
		{
			if (Msg_main[id].funproc!=NULL)
			{
				Msg_main[id].funproc();	
				Msg_main[id].Is_ready = NO;
			}
		}
	}
}
